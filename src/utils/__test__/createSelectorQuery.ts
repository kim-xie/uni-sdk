...
export class MockSelectorQueryImpl {
  ...
  set(
    ...
    const ctx = contextType === 'SelectorQuery' ? this.selectorQueryContext : this.nodesRefContext
    ctx[method] = jest.fn((...args: any[]) => {
      if (impl) {
        impl(...args)
      }
      return returnedType === 'SelectorQuery' ? this.selectorQueryContext : this.nodesRefContext
    })
